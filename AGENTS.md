...
**SSHAgentCommunicator.swift** — Pure Swift. Opens a Unix domain socket to `$SSH_AUTH_SOCK` via Darwin POSIX APIs and speaks the SSH agent wire protocol directly: `SSH_AGENTC_REQUEST_IDENTITIES (0x0b)` to list keys, `SSH_AGENTC_REMOVE_IDENTITY (0x12)` to remove a single key by blob, `SSH_AGENTC_REMOVE_ALL_IDENTITIES (0x13)` to clear all. Protocol framing: 4-byte big-endian length prefix + payload. `getLoadedKeys()` returns `[SSHKey]` directly (SHA-256 fingerprints computed via CryptoKit; raw key blob stored for use with `removeKey(blob:)`). `openSocket()` sets a 2-second `SO_RCVTIMEO`/`SO_SNDTIMEO` on the connected socket so a wedged ssh-agent fails fast instead of hanging the caller indefinitely — notably `AgentSupervisor.removeKeysOnTermination()`, which runs under app-termination's limited time budget.
